...
export async function loadInitialData(): Promise<{
  ...
  const results = await safeMultiGet([
    KEYS.notes,
    KEYS.folders,
    KEYS.profile,
    KEYS.tasks,
  ...
  if (!rawNotes) {
    ...
    await safeMultiSet([
      ...
      [KEYS.routines, JSON.stringify(SEED_ROUTINES)],
    ]);
    return {
      notes: SEED_NOTES,
  ...
  try {
    ...
    const routines = rawRoutines
      ? JSON.parse(rawRoutines).map((r: Routine) => ({
          ...r,
          links: r.links ?? {},
    ...
    return { notes, folders, profile, tasks, routines };
  } catch (e) {
